...
namespace PortalWeb_API.Controllers
{
    ...
    public class ServiciosController : ControllerBase
    {
        ...
        public async Task<IActionResult> IngresarEquipo([FromBody] EquiposTemporalesResponse model)
        {
            ...
            if (!equipoExists)
            {
                bool equipoTemporalExists = _context.EquiposTemporales.Any(ut => ut.serieEquipo == model.SerieEquipo);
                bool IPMachineSolicitudExists = _context.EquiposTemporales.Any(ut => ut.IpEquipo == model.IPMachineSolicitud);
                if (!equipoTemporalExists && !IPMachineSolicitudExists)
                {
                ...
                else
                {
                    var equipoTemporal = _context.EquiposTemporales
                        .FirstOrDefault(ut => ut.serieEquipo == model.SerieEquipo && ut.IpEquipo == model.IPMachineSolicitud);
                    if (equipoTemporal != null && equipoTemporal.Active == "F")
                    {
            ...
            try
            {
                ...
                if (verificacion)
                {
                    bool transactionExists = _context.Depositos
                                            .Where(d => d.FechaTransaccion >= model.Time_generated.AddDays(-2))
                    ...
                    if (!transactionExists)
                    {
                        ...
                        if (equipoExistente != null)
                        {
                            ...
                            else if (equipoExistente.FechaUltimaTransaccion > model.Time_generated)
                            {
                                ...
                                equipoExistente.TotalCuadreEquipo = 0;
                            }
                            else
                            {
                                equipoExistente.UltimaTransaccion = model.Transaction_no;
                ...
                if (verificacion)
                {
                    ...
                    if (!transactionExists)
                    {
                        ...
                        var update = _context.Equipos
                              .Where(u => u.SerieEquipo.Equals(model.Machine_Sn))
                              .ExecuteUpdate(u => u.SetProperty(u => u.UltimaRecoleccion, model.Time_generated));*/
                        //var equipoExistente = await _context.Equipos.FindAsync(model.Machine_Sn);
                        var equipoExistente = await _context.Equipos.FirstAsync(e => e.serieEquipo == model.Machine_Sn);
                        if (equipoExistente != null)
                        {
                            equipoExistente.ultimaRecoleccion = model.Time_generated;                            
